...
// Source file:   /home/nex/PX4-Autopilot/src/drivers/cyphal/legacy_data_types/legacy/protocol/HardwareVersion.1.0.uavcan
// Generated at:  2024-11-12 20:12:20.816280 UTC
// Is deprecated: no
// Fixed port-ID: None
...
static_assert( NUNAVUT_SUPPORT_LANGUAGE_OPTION_TARGET_ENDIANNESS == 1693710260,
              "/home/nex/PX4-Autopilot/src/drivers/cyphal/legacy_data_types/legacy/protocol/HardwareVersion.1.0.uavcan is trying to use a serialization library that was compiled with "
    ...
    const legacy_protocol_HardwareVersion_1_0* const obj, uint8_t* const buffer,  size_t* const inout_buffer_size_bytes)
{
    ...
    {   // saturated uint8[<=255] certificate_of_authenticity
        ...
        offset_bits += 8U;
        for (size_t _index1_ = 0U; _index1_ < obj->certificate_of_authenticity.count; ++_index1_)
        {
            // Saturation code not emitted -- native representation matches the serialized representation.
            buffer[offset_bits / 8U] = (uint8_t)(obj->certificate_of_authenticity.elements[_index1_]);  // C std, 6.3.1.3 Signed and unsigned integers
            offset_bits += 8U;
        }
    }
...
static inline int8_t legacy_protocol_HardwareVersion_1_0_deserialize_(
    legacy_protocol_HardwareVersion_1_0* const out_obj, const uint8_t* buffer, size_t* const inout_buffer_size_bytes)
{
    ...
    for (size_t _index2_ = 0U; _index2_ < 16UL; ++_index2_)
    {
        if ((offset_bits + 8U) <= capacity_bits)
        {
            out_obj->unique_id[_index2_] = buffer[offset_bits / 8U] & 255U;
        }
        else
        {
            out_obj->unique_id[_index2_] = 0U;
        }
        offset_bits += 8U;
    }
    ...
    for (size_t _index3_ = 0U; _index3_ < out_obj->certificate_of_authenticity.count; ++_index3_)
    {
        if ((offset_bits + 8U) <= capacity_bits)
        {
            out_obj->certificate_of_authenticity.elements[_index3_] = buffer[offset_bits / 8U] & 255U;
        }
        else
        {
            out_obj->certificate_of_authenticity.elements[_index3_] = 0U;
        }
        offset_bits += 8U;
    }
