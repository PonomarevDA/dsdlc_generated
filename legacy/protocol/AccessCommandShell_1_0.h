...
// Source file:   /home/nex/PX4-Autopilot/src/drivers/cyphal/legacy_data_types/legacy/protocol/AccessCommandShell.1.0.uavcan
// Generated at:  2024-11-12 20:12:20.805804 UTC
// Is deprecated: no
// Fixed port-ID: None
...
static_assert( NUNAVUT_SUPPORT_LANGUAGE_OPTION_TARGET_ENDIANNESS == 1693710260,
              "/home/nex/PX4-Autopilot/src/drivers/cyphal/legacy_data_types/legacy/protocol/AccessCommandShell.1.0.uavcan is trying to use a serialization library that was compiled with "
    ...
    const legacy_protocol_AccessCommandShell_Request_1_0* const obj, uint8_t* const buffer,  size_t* const inout_buffer_size_bytes)
{
    ...
    {   // saturated uint8[<=128] input
        ...
        offset_bits += 8U;
        for (size_t _index0_ = 0U; _index0_ < obj->input.count; ++_index0_)
        {
            // Saturation code not emitted -- native representation matches the serialized representation.
            buffer[offset_bits / 8U] = (uint8_t)(obj->input.elements[_index0_]);  // C std, 6.3.1.3 Signed and unsigned integers
            offset_bits += 8U;
        }
    }
...
static inline int8_t legacy_protocol_AccessCommandShell_Request_1_0_deserialize_(
    legacy_protocol_AccessCommandShell_Request_1_0* const out_obj, const uint8_t* buffer, size_t* const inout_buffer_size_bytes)
{
    ...
    for (size_t _index1_ = 0U; _index1_ < out_obj->input.count; ++_index1_)
    {
        if ((offset_bits + 8U) <= capacity_bits)
        {
            out_obj->input.elements[_index1_] = buffer[offset_bits / 8U] & 255U;
        }
        else
        {
            out_obj->input.elements[_index1_] = 0U;
        }
        offset_bits += 8U;
    }
...
static inline int8_t legacy_protocol_AccessCommandShell_Response_1_0_deserialize_(
    legacy_protocol_AccessCommandShell_Response_1_0* const out_obj, const uint8_t* buffer, size_t* const inout_buffer_size_bytes)
{
    if ((out_obj == NULL) || (inout_buffer_size_bytes == NULL) || ((buffer == NULL) && (0 != *inout_buffer_size_bytes)))
    {
        return -NUNAVUT_ERROR_INVALID_ARGUMENT;
