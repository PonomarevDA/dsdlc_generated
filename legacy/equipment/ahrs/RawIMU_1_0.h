...
// Source file:   /home/nex/PX4-Autopilot/src/drivers/cyphal/legacy_data_types/legacy/equipment/ahrs/RawIMU.1.0.uavcan
// Generated at:  2024-11-12 20:12:20.864478 UTC
// Is deprecated: no
// Fixed port-ID: None
...
static_assert( NUNAVUT_SUPPORT_LANGUAGE_OPTION_TARGET_ENDIANNESS == 1693710260,
              "/home/nex/PX4-Autopilot/src/drivers/cyphal/legacy_data_types/legacy/equipment/ahrs/RawIMU.1.0.uavcan is trying to use a serialization library that was compiled with "
    ...
    const legacy_equipment_ahrs_RawIMU_1_0* const obj, uint8_t* const buffer,  size_t* const inout_buffer_size_bytes)
{
    ...
    {   // saturated float16[3] rate_gyro_latest
        ...
        {
            float _sat0_ = obj->rate_gyro_latest[_index0_];
            if (isfinite(_sat0_))
            {
    ...
    {   // saturated float32[3] rate_gyro_integral
        ...
        for (size_t _index1_ = 0U; _index1_ < 3UL; ++_index1_)
        {
            // Saturation code not emitted -- assume the native representation of float32 is conformant.
            static_assert(NUNAVUT_PLATFORM_IEEE754_FLOAT, "Native IEEE754 binary32 required. TODO: relax constraint");
            const int8_t _err3_ = nunavutSetF32(&buffer[0], capacity_bytes, offset_bits, obj->rate_gyro_integral[_index1_]);
            if (_err3_ < 0)
            {
                return _err3_;
            }
    ...
    {   // saturated float16[3] accelerometer_latest
        ...
        {
            float _sat1_ = obj->accelerometer_latest[_index2_];
            if (isfinite(_sat1_))
            {
    ...
    {   // saturated float32[3] accelerometer_integral
        ...
        for (size_t _index3_ = 0U; _index3_ < 3UL; ++_index3_)
        {
            // Saturation code not emitted -- assume the native representation of float32 is conformant.
            static_assert(NUNAVUT_PLATFORM_IEEE754_FLOAT, "Native IEEE754 binary32 required. TODO: relax constraint");
            const int8_t _err5_ = nunavutSetF32(&buffer[0], capacity_bytes, offset_bits, obj->accelerometer_integral[_index3_]);
            if (_err5_ < 0)
            {
                return _err5_;
            }
    ...
    {   // saturated float16[<=36] covariance
        ...
        offset_bits += 8U;
        for (size_t _index4_ = 0U; _index4_ < obj->covariance.count; ++_index4_)
        {
            float _sat2_ = obj->covariance.elements[_index4_];
            if (isfinite(_sat2_))
            {
            ...
            const int8_t _err6_ = nunavutSetF16(&buffer[0], capacity_bytes, offset_bits, _sat2_);
            if (_err6_ < 0)
            {
                return _err6_;
            }
...
static inline int8_t legacy_equipment_ahrs_RawIMU_1_0_deserialize_(
    legacy_equipment_ahrs_RawIMU_1_0* const out_obj, const uint8_t* buffer, size_t* const inout_buffer_size_bytes)
{
    ...
    // saturated float16[3] rate_gyro_latest
    for (size_t _index5_ = 0U; _index5_ < 3UL; ++_index5_)
    {
        out_obj->rate_gyro_latest[_index5_] = nunavutGetF16(&buffer[0], capacity_bytes, offset_bits);
        offset_bits += 16U;
    }
    ...
    // saturated float32[3] rate_gyro_integral
    for (size_t _index6_ = 0U; _index6_ < 3UL; ++_index6_)
    {
        out_obj->rate_gyro_integral[_index6_] = nunavutGetF32(&buffer[0], capacity_bytes, offset_bits);
        offset_bits += 32U;
    }
    ...
    // saturated float16[3] accelerometer_latest
    for (size_t _index7_ = 0U; _index7_ < 3UL; ++_index7_)
    {
        out_obj->accelerometer_latest[_index7_] = nunavutGetF16(&buffer[0], capacity_bytes, offset_bits);
        offset_bits += 16U;
    }
    ...
    {
        out_obj->accelerometer_integral[_index8_] = nunavutGetF32(&buffer[0], capacity_bytes, offset_bits);
        offset_bits += 32U;
    }
    ...
    for (size_t _index9_ = 0U; _index9_ < out_obj->covariance.count; ++_index9_)
    {
        out_obj->covariance.elements[_index9_] = nunavutGetF16(&buffer[0], capacity_bytes, offset_bits);
        offset_bits += 16U;
    }
